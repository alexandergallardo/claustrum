-- 0055_academic_terms_by_plan.sql
-- Returns academic terms filtered by the modalities present in a study plan's courses.
...
plan_modalities AS (
  ...
  WHERE co.course_id IN (SELECT course_id FROM plan_courses)
...
COMMENT ON FUNCTION public.get_academic_terms_by_plan IS 'Returns academic terms for modalities that match the courses in a study plan. Falls back to all terms when no offering history exists.';
