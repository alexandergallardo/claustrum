-- FUNCTION: get_schedule_courses_by_study_plan (invoker)
-- Returns schedule courses filtered by study plan with optional other campuses support.
-- Shows one course per course_code with all its groups combined.

DROP FUNCTION IF EXISTS public.get_schedule_courses_by_study_plan(BIGINT, BIGINT, BIGINT, BOOLEAN);
...
BEGIN
  ...
  schedule_courses AS (
    ...
    JOIN plan_courses pc ON pc.course_id = v.course_id
...
COMMENT ON FUNCTION public.get_schedule_courses_by_study_plan IS 'Returns schedule courses filtered by study plan with optional other campuses support.';
