...
-- Unified schedule query for authenticated and anonymous users.
-- - If p_user_id is NULL: returns courses by study plan or academic unit.
-- - If p_user_id is set and p_study_plan_id is set: applies eligibility rules using selected plan.
-- - If p_user_id is set and only academic unit is set: returns courses by academic unit (no eligibility).

DROP FUNCTION IF EXISTS public.get_schedule_courses(UUID, BIGINT, BIGINT, BIGINT, BIGINT, BOOLEAN, BOOLEAN);
