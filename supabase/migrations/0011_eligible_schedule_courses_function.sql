-- FUNCTION: get_eligible_schedule_courses (invoker)
-- Returns schedule courses filtered by user's study plan with eligibility filtering.
-- Optionally shows only courses where prerequisites and corequisites are met.
-- Orders by level_number, then sort_order within each level.
DROP FUNCTION IF EXISTS public.get_eligible_schedule_courses(UUID, BIGINT, BIGINT, BOOLEAN, BOOLEAN);
...
COMMENT ON FUNCTION public.get_eligible_schedule_courses IS 'Returns schedule courses filtered by study plan with optional eligibility filtering. Orders by level_number, then sort_order.';
