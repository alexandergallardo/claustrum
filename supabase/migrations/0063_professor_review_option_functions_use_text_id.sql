-- 0063_professor_review_option_functions_use_text_id.sql
-- Avoid bigint precision issues in clients by accepting professor id as text.

BEGIN;

DROP FUNCTION IF EXISTS public.get_professor_review_courses(BIGINT, INTEGER);
DROP FUNCTION IF EXISTS public.get_professor_offering_terms(BIGINT, INTEGER);
...
AS $$
  ...
  WHERE EXISTS (
    ...
    WHERE cogp.professor_id::text = p_professor_id
