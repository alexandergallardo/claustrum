-- 0070_course_detail_group_card_source_course.sql
...
COMMENT ON FUNCTION public.get_course_latest_term_groups IS 'Returns groups for a course in the selected academic term, or latest term with offerings when no term is provided. For placeholder courses, includes equivalents within the selected study plan and indicates source course for each group.';
