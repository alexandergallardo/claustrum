-- 0056_academic_terms_by_plan_include_equivalences.sql
-- Extends get_academic_terms_by_plan to include equivalent courses with real names
-- when inferring plan modalities. This ensures placeholder courses (like FH1000, SE1100)
-- whose equivalents have offering history contribute their modality to the filter.
...
WHERE EXISTS (
  ...
  OR NOT EXISTS (SELECT 1 FROM plan_modalities)
ORDER BY at.year DESC, at.period_number DESC;
$$;

COMMENT ON FUNCTION public.get_academic_terms_by_plan IS 'Returns academic terms for modalities that match the courses in a study plan, including equivalent courses with real names. Falls back to all terms when no offering history exists.';
