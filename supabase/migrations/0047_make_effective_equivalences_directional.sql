-- 0047_make_effective_equivalences_directional.sql
-- Treat EQUIVALENT relations as directional: from_course_id is satisfied by
-- approved attempts for to_course_id, but not the other way around.
...
SELECT
  ...
  COALESCE(approved_origin.attempt_number, direct_latest.attempt_number) AS origin_attempt_number,
  COALESCE(approved_origin.academic_term_id, direct_latest.academic_term_id) AS origin_academic_term_id,
  COALESCE(approved_origin.academic_term_name, direct_latest.academic_term_name) AS origin_academic_term_name,
  CASE
...
IS 'Returns effective per-course status for a user/plan, propagating only APPROVED by same course globally or plan-scoped directional transitive equivalents, with origin metadata.';
...
IS 'Returns one effective status per course, preserving APPROVED globally by same course or plan-scoped directional equivalents.';
