-- 0135_include_in_progress_schedule_courses.sql
...
BEGIN
  ...
  IF p_study_plan_id IS NOT NULL THEN
    ...
    candidate_courses AS (
      ...
      WHERE cs.status IS NULL OR cs.status IN ('FAILED', 'WITHDRAWN', 'IN_PROGRESS')
    ),
    prereq_status AS (
      SELECT
...
COMMENT ON FUNCTION public.get_schedule_courses(
  ...
  BOOLEAN,
  BOOLEAN
) IS 'Unified schedule query for anonymous and authenticated users including IN_PROGRESS courses.';
