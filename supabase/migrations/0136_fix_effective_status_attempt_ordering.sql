-- 0136_fix_effective_status_attempt_ordering.sql
-- Fixes an issue where FAILED attempts were given static precedence over newer IN_PROGRESS attempts.
-- Ensures that APPROVED attempts always take priority, but for non-approved attempts,
-- the most recent attempt (by recorded_at DESC, attempt_number DESC) determines the current status.
...
propagated_attempts AS (
  ...
  ORDER BY
    scr.course_id,
    CASE WHEN scr.status = 'APPROVED' THEN 0 ELSE 1 END ASC,
...
ranked_pa AS (
  SELECT 
    *,
    ROW_NUMBER() OVER (
      ORDER BY 
        CASE WHEN status = 'APPROVED' THEN 0 ELSE 1 END ASC,
    ...
    *, 
    ROW_NUMBER() OVER (
      ORDER BY 
        ...
        CASE WHEN status = 'APPROVED' THEN 0 WHEN status = 'IN_PROGRESS' THEN 1 WHEN status = 'FAILED' THEN 2 ELSE 3 END ASC, -- Best statuses first
...
IS 'Returns effective per-course status for a user/plan. Correctly selects the latest attempt by date for non-approved courses, so newer IN_PROGRESS attempts override older FAILED attempts.';
