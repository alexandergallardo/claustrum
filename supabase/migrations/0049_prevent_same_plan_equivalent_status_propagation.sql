-- 0049_prevent_same_plan_equivalent_status_propagation.sql
-- Do not propagate statuses from equivalent courses that are also active courses
-- in the currently viewed study plan. Same-course global propagation still applies.
...
IS 'Returns effective per-course status for a user/plan, propagating APPROVED and IN_PROGRESS by same course globally or plan-scoped directional equivalents that are not active courses in the same viewed plan, with origin metadata.';
...
IS 'Returns one effective status per course, preserving APPROVED and IN_PROGRESS globally by same course or plan-scoped directional equivalents that are not active courses in the same viewed plan.';
