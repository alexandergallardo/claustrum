-- 0025_security_lint_fixes.sql
-- Fix security lint warnings:
-- 1) function_search_path_mutable for public.is_real_professor_name
  ...
  filtered AS (
    SELECT
      ...
      CASE
        ...
        ELSE GREATEST(
          extensions.similarity(p.full_name, trim(p_query)),
          CASE
    ...
    WHERE public.is_real_professor_name(p.full_name)
      AND (
        ...
        OR extensions.similarity(p.full_name, trim(p_query)) > 0.2
      )
      AND (
