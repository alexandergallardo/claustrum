...
function SettingsSection({ title, description, children, className }: SettingsSectionProps) {
  return (
    <section className={cn("grid gap-4 py-8 first:pt-0 last:pb-0 md:grid-cols-[180px_minmax(0,1fr)] md:gap-10", className)}>
      <div className="space-y-1.5">
        <h3 className="text-sm font-medium tracking-tight">{title}</h3>
        <p className="max-w-40 text-sm leading-6 text-muted-foreground">{description}</p>
      </div>
