...
export default function CalendarBody() {
  const { mode, hourHeight, setHourHeight, dayWidth, setDayWidth } =
    useCalendarContext()
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (event: TouchEvent) => {
      ...
      const nextHeight = Math.max(
        MIN_HOUR_HEIGHT,
        Math.min(MAX_HOUR_HEIGHT, startHeight * scale)
      )
      const nextWidth = Math.max(
        MIN_DAY_WIDTH,
        Math.min(MAX_DAY_WIDTH, startWidth * scale)
      )
  ...
  return (
    <div ref={containerRef} className="flex-1 flex flex-col touch-pan-x touch-pan-y">
