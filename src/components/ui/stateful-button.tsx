...
const MIN_SPINNER_VISIBLE_MS = 800;
...
const StatefulButton: React.FC<StatefulButtonProps> = ({
  ...
  const handleClick = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const waitForMinimumSpinner = async () => {
      if (buttonType !== "spinner") return;
      const elapsedMs = Date.now() - startedAt;
      const remainingMs = MIN_SPINNER_VISIBLE_MS - elapsedMs;
      if (remainingMs > 0) {
