...
function CourseFlowNode({ data }: { data: CourseNodeData }) {
  const { course, isCentral, showLeftHandle, showRightHandle, showTopHandle, showBottomHandle } =
    data;
...
export function CourseRelationFlow({
  ...
  return (
    <div
      ref={containerRef}
      className="border-border bg-card/30 aspect-[3/2] max-h-[500px] min-h-[240px] w-full overflow-hidden rounded-xl border"
    >
      <ReactFlow
        nodes={nodes}
        edges={edges}
